...
function es () {
  var q = prompt('Entscheidsuche: \nErlass Artikel (z.B. BGB 75).').toUpperCase().replace(" V", " Abs. 5").replace(" IV", " Abs. 4").replace(" III", " Abs. 3").replace(" II", " Abs. 2").replace(" I", " Abs. 1").replace(/([0-9]) ([0-9])/, "$1~$2").replace(/ +$/, "").replace(/([A-Za-z]+) ([0-9a-z]+)( Abs. [0-9])?/, "\"Art. $2\" AND \"$2$3 $1\"~5").replace(/([0-9])~([0-9])/, "$2"); 
